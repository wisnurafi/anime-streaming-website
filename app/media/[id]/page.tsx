...
export async function generateMetadata({ params }: { params: { id: number } }) {
  try {
    ...
    // Pastikan mediaData dan mediaData.title ada sebelum mengakses propertinya
    const title = mediaData?.title?.romaji || mediaData?.title?.native || 'Default Title';
    const description = mediaData?.description || `See more info about ${mediaData?.title?.romaji || mediaData?.title?.native || 'Default Title'}`;

    return {
      title: `${title} | AniProject`,
      description,
    };
  } catch (error) {
    console.error('Error fetching media data:', error);
    return {
      title: 'Default Title | AniProject',
