...
export async function optimizedFetchOnGoGoAnime({ textToSearch, only, isDubbed }: { textToSearch: string, only?: "episodes", isDubbed?: boolean }) {
    ...
    let closestResultsByMediaTitle: { id: any }[]

    if (resultsForMediaSearch && Array.isArray(resultsForMediaSearch)) {
        if (isDubbed) {
            ...
            closestResultsByMediaTitle = resultsForMediaSearch.filter(
                media => media.title && stringToOnlyAlphabetic(media.title).toLowerCase().indexOf(titleFixed) !== -1
            );  
        }
    } else {
        console.error('resultsForMediaSearch bukan array atau null');
        closestResultsByMediaTitle = []; // Atau nilai default lain yang sesuai
    }
